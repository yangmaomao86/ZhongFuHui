var app = angular.module("app", ["ngRoute", "ngFileUpload", "tm.pagination"], function ($httpProvider) {
...
app.directive('string2int', function () {
    return {
        require: 'ngModel',
...
app.filter("FormatDateDetail2", function () {
    return function (val) {
        if (val != null) {
            var date = new Date(parseInt(val.replace("/Date(", "").replace(")/", ""), 10));
            //月份为0-11，所以+1，月份小于10时补个0
...
app.filter("setHeTongLeiXing", function () {
    return function (val) {
        if (val === "1") {
            return "销售合同";
        } else if (val === "2") {
